...
- 支持 Windows/WSL 启动器，自动加载 unpacked extension。
- 可选桌面快捷方式。
...
- 在 `127.0.0.1` 启动本地 relay server。
- 把 extension 复制到 Windows 本地 app data 目录。
- 默认用专用 profile 打开 Edge。
- 通过浏览器启动参数加载 unpacked extension。
